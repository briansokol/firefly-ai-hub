...
import { createWebToolsHttpServer } from '../src/web/http.js';
...
async function startServer(config: Config) {
  const server = createWebToolsHttpServer(TOKEN, config);
...
describe('web tools http server', () => {
  let server: Server;
  let baseUrl: string;
  ...
  describe('GET /web/search (not configured)', () => {
    ...
    let base2: string;
    beforeEach(async () => {
      const started = await startServer(makeConfig({ web_search: undefined }));
      s2 = started.server;
